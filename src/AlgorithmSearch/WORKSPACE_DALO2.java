...
public class WORKSPACE_DALO2 extends AgentVariableSearch implements SelfCounterable {
    ...
    enum Status {
        ...
        createKoptAfterLocalViewUpdate, sendUnlockAndStartRelockTimerBecauseGotReject, sendUnlock, idle
    }
    ...
    public WORKSPACE_DALO2(int dcopId, int d, int agentId) {
    ...
    public boolean updateMessageInContext(MsgAlgorithm msgAlgorithm) {
        ...
        //   this.acceptMap.remove(msgAlgorithm.getSenderId());

        //}

        if (MainSimulator.isDalo2Debug) {

            if (msgAlgorithm instanceof MsgDALOInfo == false && msgAlgorithm instanceof MsgValueAssignmnet == false) {
    ...
    protected void changeReceiveFlagsToTrue(MsgAlgorithm msgAlgorithm) {
        ...
        if (this.myStatues == Status.waitForClockOrLock && msgAlgorithm instanceof MsgDALOAccept == false){
        ...
        if (msgAlgorithm instanceof MsgDALOReject){
            this.myStatues = Status.sendUnlockAndStartRelockTimerBecauseGotReject;
    ...
    public boolean getDidComputeInThisIteration() {
        return this.myStatues == Status.createInitKInfo || this.myStatues == Status.createKopt || this.myStatues == Status.createInitKInfoAndKopt        ||
                ...
                || this.myStatues == Status.createKoptAfterLocalViewUpdate ||this.myStatues == Status.sendUnlockAndStartRelockTimerBecauseGotReject ||this.myStatues == Status.sendUnlock;
    ...
    public void sendMsgs() {
        ...
        if (this.myStatues == Status.sendUnlockAndStartRelockTimerBecauseGotReject ){
            sendSelfTimerMsg();
            sendUnlock();
    ...
    private void updateStatusGivenMsgType(MsgAlgorithm msgAlgorithm) {
       ...
       // if (msgAlgorithm instanceof MsgDALOAccept || msgAlgorithm instanceof  MsgDALOSelfTimerMsg){
         //   return;
        //}
        //if (msgAlgorithm instanceof MsgValueAssignmnet){
          //  this.myStatues = Status.sendLocalViewUpdate;
        //}
            ...
            throw new RuntimeException("msg type recieve doesnt make sense, tostring: "+ msgAlgorithm.toString()+ "|| my status is: "+this.myStatues);
        }
    ...
    private void computeUpdate2Opt() {
        ...
        if (isCreating2Opt){
            ...
            this.twoOpt = new Find2Opt(makeLocalOptInfo(), this.infoOfAgents.get(selectedPartnerNodeId));
